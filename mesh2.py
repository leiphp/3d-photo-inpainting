...
def tear_edges(mesh, threshold = 0.00025, xy2depth=None):
    ...
    mesh._node = mesh._node
    for edge in mesh.edges:
        if abs(mesh._node[edge[0]]['disp'] - mesh._node[edge[1]]['disp']) > threshold:
            remove_edge_list.append((edge[0], edge[1]))

            near, far = edge if abs(edge[0][2]) < abs(edge[1][2]) else edge[::-1]

            mesh._node[far]['near'] = [] if mesh._node[far].get('near') is None else mesh._node[far]['near'].append(near)
            mesh._node[near]['far'] = [] if mesh._node[near].get('far') is None else mesh._node[near]['far'].append(far)
        ...
        if remove_flag is False:
            for (nx, ny, nd) in cc:
                info_on_pix[(nx, ny)] = [{'depth':nd,
                                          'color':mesh._node[(nx, ny, nd)]['color'],
                                          'synthesis':False,
                                          'disp':mesh._node[(nx, ny, nd)]['disp']}]
        else:
            [remove_nodes.append((nx, ny, nd)) for (nx, ny, nd) in cc]

    for node in remove_nodes:
        far_nodes = [] if mesh._node[node].get('far') is None else mesh._node[node]['far']
        for far_node in far_nodes:
            if mesh.has_node(far_node) and mesh._node[far_node].get('near') is not None and node in mesh._node[far_node]['near']:
                mesh._node[far_node]['near'].remove(node)
        near_nodes = [] if mesh._node[node].get('near') is None else mesh._node[node]['near']
        for near_node in near_nodes:
            if mesh.has_node(near_node) and mesh._node[near_node].get('far') is not None and node in mesh._node[near_node]['far']:
                mesh._node[near_node]['far'].remove(node)
...
def generate_face(mesh, info_on_pix, config):
    ...
    mesh._node = mesh._node
    for node in mesh._node:
        cur_id_self = mesh._node[node]['cur_id']
        ne_nodes = get_neighbors(mesh, node)
        four_dir_nes = {'up': [], 'left': [],
                        'down': [], 'right': []}
        for ne_node in ne_nodes:
            store_tuple = [ne_node, mesh._node[ne_node]['cur_id']]
            if ne_node[0] == node[0]:
            ...
            for ne in get_cross_nes(lost_x, lost_y):
                if key_exist(info_on_pix, ne):
                    for info in info_on_pix[ne]:
                        ...
                        if key_exist(mesh._node[ne_node], 'edge_id'):
                            edge_id = mesh._node[ne_node]['edge_id']
...
def combine_end_node(mesh, edge_mesh, edge_ccs, depth):
    ...
    for valid_edge_id, valid_edge_cc in enumerate(edge_ccs):
        ...
        for valid_edge_node in valid_edge_cc:
            ...
            for ne_node in mesh.neighbors(valid_edge_node):
                if mesh._node[ne_node].get('far') is not None:
                    for fn in mesh._node[ne_node].get('far'):
                        if mesh.has_node(fn) and mesh._node[fn].get('edge_id') is not None:
                            single_connect.add(mesh._node[fn]['edge_id'])
                if mesh._node[ne_node].get('near') is not None:
                    for fn in mesh._node[ne_node].get('near'):
                        if mesh.has_node(fn) and mesh._node[fn].get('edge_id') is not None:
                            single_connect.add(mesh._node[fn]['edge_id'])
    ...
    invalid_nodes = set()
    for nx, ny in zip(nxs, nys):
        if mesh._node[(nx, ny, end_maps[nx, ny])].get('edge_id') is None:
            continue
        else:
            self_id = mesh._node[(nx, ny, end_maps[nx, ny])].get('edge_id')
            self_connect = connect_dict[self_id] if connect_dict.get(self_id) is not None else dict()
        four_nes = [xx for xx in [(nx - 1, ny), (nx + 1, ny), (nx, ny - 1), (nx, ny + 1)] \
                        if 0 <= xx[0] < mesh.graph['H'] and 0 <= xx[1] < mesh.graph['W'] and \
                        end_maps[xx[0], xx[1]] != 0]
        for fne in four_nes:
            if mesh._node[(fne[0], fne[1], end_maps[fne[0], fne[1]])].get('edge_id') is None:
                continue
            else:
                ne_id = mesh._node[(fne[0], fne[1], end_maps[fne[0], fne[1]])]['edge_id']
    ...
    for nx, ny in zip(nxs, nys):
        ...
        for fne in four_nes:
            if mesh.has_node((fne[0], fne[1], end_maps[fne[0], fne[1]])):
                ...
                mesh._node[node_b]['must_connect'] = set() if mesh._node[node_b].get('must_connect') is None else mesh._node[node_b]['must_connect']
                mesh._node[node_b]['must_connect'].add(node_a)
                mesh._node[node_b]['must_connect'] |= set([xx for xx in [*edge_mesh.neighbors(node_a)] if \
                                                            (xx[0] - node_b[0]) < 2 and (xx[1] - node_b[1]) < 2])
                mesh._node[node_a]['must_connect'] = set() if mesh._node[node_a].get('must_connect') is None else mesh._node[node_a]['must_connect']
                mesh._node[node_a]['must_connect'].add(node_b)
                mesh._node[node_a]['must_connect'] |= set([xx for xx in [*edge_mesh.neighbors(node_b)] if \
...
def context_and_holes(mesh, edge_ccs, config, specific_edge_id, specific_edge_loc, depth_feat_model,
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        edge_group = {}
        for edge_node in edge_cc:
            far_nodes = mesh._node[edge_node].get('far')
            ...
            for far_node in far_nodes:
                ...
                if mesh._node[far_node].get('edge_id') is not None:
                    if edge_group.get(mesh._node[far_node]['edge_id']) is None:
                        edge_group[mesh._node[far_node]['edge_id']] = set()
                    edge_group[mesh._node[far_node]['edge_id']].add(far_node)
        if len(edge_cc) > 2:
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        for edge_node in edge_cc:
            raw_intouched_nodes = set(mesh._node[edge_node].get('near')) if mesh._node[edge_node].get('near') is not None else set()
            tmp_intouched_nodes |= set([xx for xx in raw_intouched_nodes if mesh._node[xx].get('edge_id') is not None and \
                                                                         len(context_ccs[mesh._node[xx].get('edge_id')]) > 0])
    ...
    for edge_id, edge_cc in enumerate(edge_ccs):
        ...
        for i in range(max(background_thickness, context_thickness)):
            ...
            if i == 0:
                ...
                for node in tmp_mask_nodes:
                    ...
                    depth_count = 0
                    if mesh._node[node].get('far') is not None:
                        for comp_cnt_node in mesh._node[node]['far']:
                            comp_cnt_depth[node[0], node[1]] += abs(comp_cnt_node[2])
                            depth_count += 1
                    if depth_count > 0:
                        comp_cnt_depth[node[0], node[1]] = comp_cnt_depth[node[0], node[1]] / depth_count
                    connect_node = []
                    if mesh._node[node].get('connect_point_id') is not None:
                        connect_node.append(mesh._node[node]['connect_point_id'])
                    ...
                    if mesh._node[node].get('connect_point_exception') is not None:
                        for xx in mesh._node[node]['connect_point_exception']:
                            if connect_map[xx[0], xx[1]] == 0:
                                connect_map[xx[0], xx[1]] = xx[2]
                    ...
                    if bool(context_map[ne[0], ne[1]]) is False and \
                       ...
                       bool(intersect_context_map[ne[0], ne[1]]) is False:
                        ...
                        if i < background_thickness:
                            ...
                            elif mesh._node[ne].get('inpaint_id') == 1:
                                cur_mask_cc.append(ne)
                            else:
                                continue
                            ...
                            if inpaint_iter == 0:
                                ...
                                if mesh._node[ne].get('far') is not None:
                                    for comp_far_node in mesh._node[ne]['far']:
                                        cur_comp_far_cc.append(comp_far_node)
                                        cur_accomp_near_cc.append(ne)
                                        cur_invalid_extend_edge_cc.append(comp_far_node)
                                if mesh._node[ne].get('edge_id') is not None and \
                                    len(context_ccs[mesh._node[ne].get('edge_id')]) > 0:
                                    intouched_fars = set(mesh._node[ne].get('far')) if mesh._node[ne].get('far') is not None else set()
                                ...
                                if mesh._node[ne].get('near') is not None:
                                    intouched_nears = set(mesh._node[ne].get('near'))
                        ...
                        if not (mesh._node[ne].get('inpaint_id') != 1 and inpaint_iter == 1):
                       ...
                       bool(intersect_context_map[ne[0], ne[1]]) is False:
                        if i == passive_context and inpaint_iter == 1:
                            ...
                            if any([mask_map[mne[0], mne[1]] == True for mne in mnes]) is True:
                                ...
                                continue
                        if False and mesh._node[ne].get('near') is not None and mesh._node[ne].get('edge_id') != edge_id:
                            noncont_nears = set(mesh._node[ne].get('near'))
        ...
        for erode_context_node in erode_context_cc:
            if (inpaint_iter != 0 and (mesh._node[erode_context_node].get('inpaint_id') is None or
                                        mesh._node[erode_context_node].get('inpaint_id') == 0)):
...
def DL_inpaint_edge(mesh,
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(zip(context_ccs, mask_ccs, erode_context_ccs, extend_context_ccs, edge_ccs)):
        ...
        for dx, dy in zip(dxs, dys):
            mesh._node[(dx, dy)]['inpaint_twice'] = False
        ...
        for hx, hy in zip(hxs, hys):
            ...
            if info_on_pix.get((node[0], node[1])) is not None:
                for info in info_on_pix.get((node[0], node[1])):
                    if info.get('inpaint_id') is None or abs(info['inpaint_id'] < mesh._node[(hx, hy)]['inpaint_id']):
            ...
            if not(mesh.has_node(node)):
                ...
                mesh, _, _ = refresh_node((node[0], node[1]), mesh._node[(node[0], node[1])], node, ndict, mesh, stime=True)
        ...
        for erode_node in erode_context_cc:
            for info in info_on_pix[(erode_node[0], erode_node[1])]:
                if info['depth'] == erode_node[2]:
                    ...
                    mesh._node[erode_node]['color'] = info['update_color']
    ...
    for node in mesh._node:
        if len(node) == 2:
            ...
            continue
        if mesh._node[node].get('edge_id') is not None and mesh._node[node].get('inpaint_id') == inpaint_iter + 1:
            if mesh._node[node].get('inpaint_twice') is False:
                continue
            try:
                new_edge_ccs[mesh._node[node].get('edge_id')].add(node)
            except:
                import pdb; pdb.set_trace()
...
def write_ply(image,
    ...
    for pix_xy, pix_list in info_on_pix.items():
        for pix_idx, pix_info in enumerate(pix_list):
            ...
            input_mesh._node[(pix_xy[0], pix_xy[1], pix_info['depth'])]['cur_id'] = out_fmt(vertex_id, ply_flag)
            vertex_id += 1
            if ply_flag is True:
